...
export default {
  ...
  methods: {
    loadUsersData() {
      axios.post((import.meta.env.DEV? 'http://localhost:3070' : window.location.origin) + '/api/users')
        .then((res) => {
          this.users = res.data;
        })
    ...
    saveUsers() {
      this.editing = false;
      axios.put((import.meta.env.DEV? 'http://localhost:3070' : window.location.origin) + '/api/users', {users: this.users})
        .catch((err) => console.log(err));
    },
    discardChanges() {
      this.loadUsersData();
